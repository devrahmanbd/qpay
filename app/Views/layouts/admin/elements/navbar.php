      ...
      <div class="flex items-center gap-2">
         ...
         <div class="relative">
            <button @click="notifOpen = !notifOpen" @click.outside="notifOpen = false" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg relative">
               ...
               <span class="total_notification absolute -top-0.5 -right-0.5 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center" data-url="<?= admin_url('get_admin_notifications') ?>" style="font-size:10px"></span>
            </button>
         ...
         <div class="relative">
            <button @click="profileOpen = !profileOpen" @click.outside="profileOpen = false" class="flex items-center gap-2 p-1.5 hover:bg-gray-100 rounded-lg">
               <img alt="avatar" src="<?= get_avatar('admin') ?>" class="w-8 h-8 rounded-full object-cover">
            </button>
            <div x-show="profileOpen" x-cloak x-transition class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1">
               <div class="px-4 py-2 border-b border-gray-100">
                  <p class="text-sm font-medium text-gray-700">Hello <?= current_admin('first_name'); ?></p>
               </div>
               <a href="<?= admin_url('profile') ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                  Profile
               </a>
               <div class="border-t border-gray-100 my-1"></div>
               <a href="<?= admin_url('logout') ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50">
