...
$account_types = [
    ...
    'merchant' => ['name' => 'Merchant', 'icon' => 'fa-briefcase', 'desc' => 'Full merchant API'],
    'payment'  => ['name' => 'Payment', 'icon' => 'fa-credit-card', 'desc' => 'Direct payment mode']
];
?>
    ...
    <form action="<?= $form_url ?>" class="form actionForm space-y-8" method="POST" data-redirect="<?= current_url() ?>">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-50/50 border border-gray-100 p-4 rounded-xl">
                ...
                <div class="flex items-center justify-between">
                    <span class="text-sm font-medium text-gray-700">Enable bKash</span>
            ...
            <div class="bg-gray-50/50 border border-gray-100 p-4 rounded-xl">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="sandbox" value="1" <?= @get_value($params, 'sandbox') == 1 ? 'checked' : '' ?> class="sr-only peer">
            ...
            <!-- Debug Logs -->
            <div class="bg-gray-50/50 border border-gray-100 p-4 rounded-xl">
                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Logging</label>
                <div class="flex items-center justify-between">
                    <span class="text-sm font-medium text-gray-700">Debug Logs</span>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="hidden" name="logs" value="0">
                        <input type="checkbox" name="logs" value="1" <?= @get_value($params, 'logs') == 1 ? 'checked' : '' ?> class="sr-only peer">
                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-600"></div>
                    </label>
                </div>
            </div>
        ...
        <div x-data="{ activeType: '<?= !empty($active_payments) ? array_keys(array_filter($active_payments))[0] ?? 'personal' : 'personal' ?>' }">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-6 lg:p-8 shadow-sm">
                <!-- Personal / Agent / Payment Number -->
                <div x-show="['personal', 'agent', 'payment'].includes(activeType)" x-cloak class="space-y-6">
                    <div x-show="activeType === 'personal'">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">bKash Personal Number</label>
                        <div class="relative">
                    ...
                    <div x-show="activeType === 'agent'">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">bKash Agent Number</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-400">
                    ...
                    <div x-show="activeType === 'payment'">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">bKash Merchant Payment Number</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-400">
                                <i class="fas fa-credit-card"></i>
                            </span>
                            <input type="text" name="payment_number" value="<?= @get_value($params, 'payment_number') ?>" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all" placeholder="01XXXXXXXXX">
                        </div>
                    </div>
                </div>
                ...
                <div x-show="activeType === 'merchant'" x-cloak class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="username" value="<?= @get_value($params, 'username') ?>" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all" placeholder="Merchant Username">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Password</label>
                            <input type="password" name="password" value="<?= @get_value($params, 'password') ?>" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all" placeholder="••••••••">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">APP Key</label>
                            <input type="text" name="app_key" value="<?= @get_value($params, 'app_key') ?>" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all" placeholder="bKash App Key">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">APP Secret Key</label>
                            <input type="password" name="app_secret" value="<?= @get_value($params, 'app_secret') ?>" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all" placeholder="••••••••">
                        </div>
                    </div>
                </div>
