<section class="pt-8 pb-16">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-6">
      ...
      <span>/</span>
      <a href="<?= base_url('blogs') ?>" class="hover:text-primary-600">Blog</a>
      <span>/</span>
      <span class="text-gray-900 truncate max-w-xs"><?= $blog->title ?></span>
    </nav>
      ...
      <article class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <?php if (!empty($blog->thumbnail)) : ?>
            <img src="<?= $blog->thumbnail ?>" alt="<?= esc($blog->title) ?>" class="w-full h-64 object-cover">
          <?php endif; ?>
          <div class="p-6 lg:p-8">
            <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 mb-3"><?= $blog->title; ?></h1>
            <div class="flex items-center gap-4 text-sm text-gray-500 mb-6">
              ...
              <span class="flex items-center gap-1">
                ...
                <time><?= time_format($blog->created_at); ?></time>
              </span>
            </div>
